...
class Java:
    ...
    def _javac(self, java_file):
        p = subprocess.Popen([self.bin + 'javac', '-cp', 'jars/*', java_file], stdout=subprocess.PIPE,
    ...
    def _java(self, class_file):
        p = subprocess.Popen(
            ['isolate', '--processes=32', '--meta=meta', f'--time={TIME}', f'--wall-time={WALL_TIME}', '--stdin=in',
             '--run', self.bin + 'java', class_file[:-6]],
...
class Python:
    ...
    def _py_compile(self, python_file):
        p = subprocess.Popen([self.bin + 'python3', '-m', 'py_compile', python_file], stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE)
        _, error = p.communicate()
        return p.returncode, error.decode()
    ...
    def _python(self, python_file):
        p = subprocess.Popen(
            ['isolate', '--meta=meta', f'--time={TIME}', f'--wall-time={WALL_TIME}', '--stdin=in', '--run',
             self.bin + 'python3', python_file],
            stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        output, error = p.communicate()
        return output.decode(), error.decode()
